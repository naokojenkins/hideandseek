...
namespace HideAndSeek.Core.Config
{
    ...
    public class WorldConfig
    {
        ...
        // Параметры разрежения/постобработки лабиринта
        /// <summary>
        /// Масштаб для количества дополнительных проходов после генерации MazeDFS.
        /// Итоговое количество рассчитывается как round(GridSize * ExtraPassagesScale), затем клампится в [0, GridSize].
        /// По умолчанию 0.4 — немного больше развилок и проёмов между коридорами по сравнению с 0.3.
        /// </summary>
        public float ExtraPassagesScale { get; set; } = 0.4f;

        /// <summary>
        /// Желаемая толщина (в клетках) для дополнительных проёмов, пробиваемых на этапе extraPassages.
        /// Поддерживаются значения 1 или 2. По умолчанию 2, чтобы проёмы были хорошо различимы и
        /// не превращались в узкие «мостики» шириной 1 клетка.
        /// </summary>
        public int ExtraPassageWidth { get; set; } = 2;

        /// <summary>
        /// Использовать ли «жёсткий» нижний порог плотности (WallDensityMin) вместо «мягкого»
        /// целевого (WallDensityTargetMin) при добавлении дополнительных проёмов.
        /// Если true, extraPassages сможет открывать больше проёмов при высоких значениях
        /// ExtraPassagesScale до тех пор, пока не будет достигнут жёсткий минимум плотности.
        /// По умолчанию true, чтобы изменение масштаба проёмов давало заметный визуальный эффект.
        /// </summary>
        public bool ExtraPassagesUseHardMin { get; set; } = true;

        /// <summary>
        /// Минимальная ширина проходов. На данный момент гарантируется значение 2 (>=2),
        /// более широкие значения могут поддерживаться в будущих версиях.
        /// </summary>
        public int MinPassageWidth { get; set; } = 2;

        /// <summary>
        /// Дополнительные «перемычки» между параллельными коридорами, масштаб вероятности.
        /// 0..1, по умолчанию 0.15. Увеличивает число проёмов, не затрагивая периметр.
        /// </summary>
        public float ExtraJunctionsScale { get; set; } = 0.15f;

        /// <summary>
        /// Минимально допустимая доля стен (0..1) после постобработки. Если после карвинга узких перемычек
        /// доля стен опустится ниже этого порога, мы откатываем карвинг для сохранения структуры.
        /// По умолчанию 0.20 (20%).
        /// </summary>
        public float WallDensityMin { get; set; } = 0.28f;

        /// <summary>
        /// Включает адаптивный контроль плотности стен на этапах, где стены «прорезаются» (доп. проходы,
        /// перемычки и выравнивание ширины). При включении этапы останавливаются раньше, если текущая
        /// операция может опустить плотность ниже целевого минимума.
        /// </summary>
        public bool AdaptiveDensityControl { get; set; } = true;

        /// <summary>
        /// Желательная нижняя граница плотности внутренних стен. Адаптивные этапы стараются не опускаться
        /// ниже этой величины (жёсткий нижний порог задаёт WallDensityMin). По умолчанию 0.28.
        /// </summary>
        public float WallDensityTargetMin { get; set; } = 0.28f;

        /// <summary>
        /// Необязательная верхняя граница желаемой плотности (пока не используется логикой, но оставлена для тюнинга).
        /// </summary>
        public float WallDensityTargetMax { get; set; } = 0.50f;

        /// <summary>
        /// Включает подробные диагностические логи генерации мира (плотности, этапы и пр.). По умолчанию выключено.
        /// </summary>
        public bool DebugWorldGenLogs { get; set; } = false;

        /// <summary>
        /// Допускаемая «эпсилон‑слабина» для шагов, гарантирующих минимальную ширину проходов
        /// (EnforceMinPassageWidth/WidenLinearCorridorsToWidth). Позволяет этим шагам опуститься
        /// чуть ниже жёсткого порога плотности <see cref="WallDensityMin"/> не более чем на заданную величину,
        /// чтобы устранить остаточные коридоры шириной 1 клетка. Значение задаётся в долях (0..1),
        /// по умолчанию 0.01 (1%). На «разрежающих» шагах (extraPassages/extraJunctions) это значение
        /// не применяется — там по‑прежнему действует строгий <see cref="WallDensityMin"/>.
        /// </summary>
        public float WidthGuaranteeHardMinEps { get; set; } = 0.01f;

        // ——— Анти-«шахматность» и управление разрежением ———
        /// <summary>
        /// Включает учёт минимального расстояния между проёмами/перемычками и анти-симметричный джиттер.
        /// Позволяет избежать регулярного «шахматного» узора одиночных кубов.
        /// </summary>
        public bool EnableSpacingAwareCarving { get; set; } = true;

        /// <summary>
        /// Минимальная Манхэттен-дистанция между соседними проёмами (пустыми клетками, которые являются перемычками).
        /// </summary>
        public int JunctionMinSpacing { get; set; } = 2;

        /// <summary>
        /// Вес приоритета для кандидатов, которые уменьшают число тупиков (dead-ends), 0..1.
        /// </summary>
        public float JunctionBiasDeadEnds { get; set; } = 0.6f;

        /// <summary>
        /// Вероятность нарушить регулярный выбор (анти-симметричный джиттер) при добавлении проёмов.
        /// </summary>
        public float AntiSymmetryJitter { get; set; } = 0.15f;

        /// <summary>
        /// Целевая минимальная длина коротких сегментов стен для попытки их «склейки».
        /// </summary>
        public int WallSegmentMinLengthTarget { get; set; } = 2;
